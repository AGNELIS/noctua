...
const GROUNDING_EXERCISES = [
  { id: "54321", title: "5-4-3-2-1 Senses", duration: "3-5 min", description: "Ground yourself through your senses.", steps: ["Take a slow, deep breath.", "Look around and name 5 things you can SEE.", "Reach out and name 4 things you can TOUCH.", "Listen carefully and name 3 things you can HEAR.", "Notice 2 things you can SMELL.", "Name 1 thing you can TASTE.", "Take another deep breath. You are here. You are safe."], icon: "✋" },
  { id: "breathing", title: "Box Breathing", duration: "2-4 min", description: "Inhale, hold, exhale, hold — each for 4 counts.", steps: ["Sit comfortably and close your eyes if you wish.", "Breathe IN slowly for 4 counts.", "HOLD your breath for 4 counts.", "Breathe OUT slowly for 4 counts.", "HOLD empty for 4 counts.", "Repeat 4-6 times.", "Notice how your body feels now."], icon: "🌬" },
  { id: "bodyscan", title: "Quick Body Scan", duration: "3-5 min", description: "Bring gentle awareness to each part of your body.", steps: ["Close your eyes or soften your gaze.", "Notice your feet on the ground.", "Move awareness to your legs. Release any tension.", "Notice your belly. Let it soften with each breath.", "Feel your chest and heart space. Breathe into it.", "Relax your shoulders away from your ears.", "Soften your jaw, your forehead, the space behind your eyes.", "Take a full breath. You are whole. You are here."], icon: "🧘" },
  { id: "rooting", title: "Rooting Visualisation", duration: "2-3 min", description: "Imagine roots growing from your body into the earth.", steps: ["Stand or sit with your feet flat on the floor.", "Imagine roots growing from the soles of your feet.", "See them reaching down through the floor, into the earth.", "They grow deeper — through soil, stone, and water.", "Feel the earth energy rising back up through your roots.", "It fills your legs, your core, your whole body with warmth.", "You are rooted. You are held. You are supported."], icon: "🌳" },
  { id: "cold", title: "Cold Water Reset", duration: "1 min", description: "A quick physiological reset using cold water.", steps: ["Go to a sink or use a glass of cold water.", "Splash cold water on your face.", "Or hold a cold object in your hands.", "Focus completely on the sensation of cold.", "Breathe slowly as the cold brings you into the present.", "Notice: you are here, right now, in this moment."], icon: "💧" },
];
  ...
  return (
    <div className="min-h-screen transition-colors duration-500" style={{ backgroundColor: "var(--color-cream)" }}>
      <header className="px-6 pt-5 pb-2">
        ...
        <h1 className="text-lg md:text-xl tracking-[0.25em] uppercase text-center mt-3" style={{ color: "var(--color-plum)", fontFamily: "'Antic Didone', Georgia, serif", fontWeight: 700 }}>Grounding</h1>
      </header>
        ...
        {exercise ? (
          <section className="rounded-2xl border p-6 space-y-6 transition-colors duration-500" style={{ background: "var(--color-blush)", borderColor: "var(--color-dusty-rose)" }}>
            <div className="text-center space-y-2">
              ...
              <h2 className="text-lg font-medium" style={{ color: "var(--color-dark)" }}>{exercise.title}</h2>
              <p className="text-xs" style={{ color: "var(--color-mauve)" }}>Step {currentStep + 1} of {exercise.steps.length}</p>
            </div>
            ...
            <p className="text-center text-base leading-relaxed min-h-[4rem] flex items-center justify-center" style={{ color: "var(--color-dark)" }}>{exercise.steps[currentStep]}</p>
            <div className="flex justify-center gap-3">
              {currentStep > 0 && (
                <button onClick={() => setCurrentStep(currentStep - 1)} className="px-5 py-2.5 rounded-xl text-sm border" style={{ borderColor: "var(--color-dusty-rose)", color: "var(--color-plum)" }}>← Previous</button>
              )}
            ...
            <button onClick={() => { setActiveExercise(null); setCurrentStep(0); }} className="block mx-auto text-xs" style={{ color: "var(--color-mauve)" }}>Exit exercise</button>
          </section>
        ) : (
          <div className="space-y-6">
            <section className="space-y-3">
              <h2 className="text-xs tracking-widest uppercase text-center" style={{ color: "var(--color-mauve)" }}>Grounding Exercises</h2>
                ...
                <button key={ex.id} onClick={() => { setActiveExercise(ex.id); setCurrentStep(0); }}
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center justify-between">
                        <h3 className="text-sm font-medium" style={{ color: "var(--color-dark)" }}>{ex.title}</h3>
                        <span className="text-xs" style={{ color: "var(--color-mauve)" }}>{ex.duration}</span>
                      </div>
                      <p className="text-xs mt-1 leading-relaxed" style={{ color: "var(--color-mauve)" }}>{ex.description}</p>
                    </div>
                  </div>
                </button>
              ))}
            ...
            <section className="space-y-3">
              <button onClick={() => setShowResources(!showResources)}
                ...
                <p className="text-base font-medium" style={{ color: "var(--color-dark)" }}>Need to talk to someone?</p>
                <p className="text-xs mt-1" style={{ color: "var(--color-mauve)" }}>{showResources ? "Tap to hide" : "Tap to see support resources"}</p>
              </button>
              {showResources && (
                <div className="space-y-2">
                  <p className="text-xs text-center leading-relaxed" style={{ color: "var(--color-mauve)" }}>You are not alone. These services are confidential and available when you need them.</p>
