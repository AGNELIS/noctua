...
export async function GET(req: NextRequest) {
  ...
  const count = (arr: any[] | null, userId: string, field = "user_id") =>
    (arr || []).filter((r) => r[field] === userId).length;
  ...
  const enrichedUsers = (users || []).map((u) => {
    ...
    // Engagement score (simple v1)
    const consistencyScore = Math.min(totalEntries / 20, 1) * 30; // max 30 pts for 20+ entries
    const depthScore = Math.min(completedWorkbooks / 3, 1) * 25; // max 25 pts for 3+ workbooks
    const aiScore = Math.min((analysisCount + reportCount) / 5, 1) * 20; // max 20 pts
    const returnScore = lastSignIn ? (Date.now() - new Date(lastSignIn).getTime() < 7 * 24 * 60 * 60 * 1000 ? 15 : 5) : 0; // 15 pts if active in last 7 days
    const communityScore = Math.min(activeReferrals / 2, 1) * 10; // max 10 pts
